<template>
    <form action="">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-11">
                <textarea rows="5" class="form-control" v-model="newPizza.descripton"></textarea>
            </div>
        </div>
    ...
    data(){
     ...
     methods:{
        addMenuItem(){
            ...
            //方法2:
            //this.axios.post('menu.json',data).then(this.$router.push({name:"menuLink"}))

            //数据同步到vuex中
            this.axios.post('menu.json',data).then(res=>this.$store.commit("pushToMunuItems",data))
        }
    }
}
</script>
